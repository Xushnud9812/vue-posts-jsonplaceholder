...
const emit = defineEmits(['closeModal', 'post-creates'])
...
<template>
  <div class="fixed  z-20 w-full h-screen top-0 left-0">
    ...
    <div class="absolute bg-white p-5 rounded w-1/2 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
      ...
      <form class="flex flex-col" @submit.prevent="createPost">
        ...
        <input placeholder="title..."
          ...
          type="text" v-model="newPost.title" id="title" required>
        ...
        <textarea placeholder="body..."
          ...
          v-model="newPost.body" rows="5" id="body" required></textarea>
        ...
        <div class="mt-5 gap-2 flex justify-end items-center">
          ...
          <button type="submit"
            class="bg-yellow-500 hover:bg-yellow-400 border-0 py-1 px-3 focus:outline-none  rounded text-base ">Create</button>
        </div>
      </form>
    </div>
  </div>
